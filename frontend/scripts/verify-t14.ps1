...
$outputRoot = Join-Path $taskRoot '.tmp/t14'
New-Item -ItemType Directory -Path $outputRoot -Force | Out-Null
$results = [ordered]@{}
Push-Location (Join-Path $taskRoot 'backend')
try {
    & .\mvnw.cmd test *> (Join-Path $outputRoot 'user_backend.log')
    $results.backend = $LASTEXITCODE
} finally { Pop-Location }
Push-Location (Join-Path $taskRoot 'frontend')
try {
    & npm.cmd test -- --run *> (Join-Path $outputRoot 'user_frontend_tests.log')
    $results.frontendTests = $LASTEXITCODE
    & npm.cmd run build *> (Join-Path $outputRoot 'user_frontend_build.log')
    $results.frontendBuild = $LASTEXITCODE
} finally { Pop-Location }
$results | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $outputRoot 'validation.json') -Encoding utf8
